...
// dsh-tools package build (auto-generated from plugins/cc-file-installer/host.js by scripts/build-packages.mjs)
...
const definition = {
  ...
  apply(ctx) {
    ...
    // ---------- file engine: marketplace add/remove ----------
    async function marketplaceAddFile(source) {
      ...
      return wr.ok
        ? { ok: true, output: 'marketplace added (DSH-owned, no claude): ' + name + ' [' + srcType + '] → ' + target }
    ...
    // ---------- file engine: plugin install/remove ----------
    async function resolvePluginEntry(pluginName, marketName) {
      ...
      if (marketName) {
        ...
        return { error: 'plugin "' + pluginName + '" not found in marketplace "' + marketName + '" — run ccmarket_file update first' }
      }
    ...
    async function installFile(id) {
      ...
      return wr.ok
        ? { ok: true, output: 'installed (DSH-owned, no claude CLI): ' + pluginName + '@' + mName + ' v' + version + ' → ' + finalDir }
        : { ok: false, output: 'registry write failed: ' + wr.error }
    }
    ...
    const tools = [
      {
        name: 'ccmarket_file',
        description: 'Manage Claude Code plugin marketplaces in the DSH-owned directory (~/.dsh/ccbridge/plugins) WITHOUT the claude CLI: add a marketplace from a GitHub repo (owner/repo or URL) or a local path, remove or update one. Fully independent of ~/.claude.',
        params: {
          action: { type: 'string', enum: ['add', 'remove', 'update'], required: true, description: 'add (source required), remove (name required), update (name required — re-fetches from its recorded source).' },
          source: { type: 'string', description: 'For add: GitHub owner/repo, https URL, or local directory path. For remove/update: marketplace name.' },
        },
        safe: false,
        exec: async (args) => {
          const action = String(args.action || '')
          if (action === 'add') {
            const r = await marketplaceAddFile(args.source)
            return r.ok ? r.output : 'FAILED — ' + r.output
          }
          if (action === 'remove') {
            const r = await marketplaceRemoveFile(String(args.source || ''))
            return r.ok ? r.output : 'FAILED — ' + r.output
          }
          const r = await marketplaceUpdateFile(String(args.source || ''))
          return r.ok ? r.output : 'FAILED — ' + r.output
        },
      },
      {
        name: 'ccinstall_file',
        description: 'Install a Claude Code plugin into the DSH-owned directory WITHOUT the claude CLI or ~/.claude: resolves the plugin entry from a DSH-managed marketplace, copies local sources or git-clones url/git-subdir sources at their pinned ref/sha, writes the version directory under ~/.dsh/ccbridge/plugins/cache and records it in the DSH registry (~/.dsh/ccbridge/plugins/installed_plugins.json).',
        params: {
          plugin: { type: 'string', required: true, description: 'Plugin id: name, or name@marketplace (recommended).' },
          scope: { type: 'string', enum: ['user'], description: 'Installation scope (file engine supports user).' },
        },
        safe: false,
        exec: async (args) => {
          const r = await installFile(args.plugin, args.scope)
          return r.ok ? r.output : 'FAILED — ' + r.output
        },
      },
      {
        name: 'ccuninstall_file',
        description: 'Remove an installed plugin from the DSH-owned registry (deletes its directory under ~/.dsh/ccbridge/plugins/cache and its record). Does not touch ~/.claude.',
        params: {
          plugin: { type: 'string', required: true, description: 'Plugin id, e.g. name@marketplace.' },
        },
        safe: false,
        exec: async (args) => {
          const r = await removeFile(String(args.plugin || '').trim())
          return r.ok ? r.output : 'FAILED — ' + r.output
        },
      },
      {
        name: 'ccrescan_skills',
        description: 'Force the DSH skill catalog to re-collect after plugin changes. The registry caches its catalog, so newly installed plugins only become loadable with the `skill` tool after this refresh (registering a transient skill invalidates the cache).',
        params: {},
        safe: true,
        exec: async () => {
          ...
          return 'skill catalog invalidated'
        },
      },
    ]
