  ...
  apply(ctx) {
    ...
    function MarketPage() {
      ...
      const refresh = async () => {
        setBusy(true); setErr('')
        try {
          const r = await host.call('cm.list', {})
          setData((r && r.markets) || [])
        } catch (e) {
      ...
      const doPlugin = async (action, id) => {
        ...
        try {
          const r = await host.call('cm.' + action, { id: id })
      ...
      const doMarket = async (action, src) => {
        ...
        try {
          const r = await host.call('cm.market', { action: action, source: src || undefined })
      ...
      const doRescan = async () => {
        setBusy(true); setErr('')
        try {
          const r = await host.call('cm.rescan', {})
